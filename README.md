...
*   **업데이트 시간:** 매주 **화요일 오전 9시 ET** (미국 동부 시간 기준).
*   **한국 시간(KST) 변환:**
    *   서머타임 적용 시 (3월~11월): 매주 **화요일 밤 10시 KST**.
    *   서머타임 미적용 시: 매주 **화요일 밤 11시 KST**.
*   **데이터 특성:** 주 단위로 대량 업데이트되나, 항구별로 업데이트 시점이 미세하게 다를 수 있습니다.

### 2. 권장 실행 스케줄
*   **매일 1회 실행 권장:** 데이터는 주 단위로 몰아서 올라오지만, IMF 측의 데이터 지연이나 개별 보정 사항을 즉시 반영하기 위해 **매일 새벽(한국 시간)**에 정기적으로 스크립트를 실행하는 것이 가장 안전합니다.
*   **UPSERT 방식:** 현재 모든 적재 로직은 UPSERT(MERGE INTO)를 지원하므로, 매일 실행해도 중복 데이터가 쌓이지 않으며 변경된 내역만 효율적으로 갱신합니다.

---

## ⚠️ 이슈 리포트
*   **AISStream Cloudflare Issue:** 현재 `aisstream.io` 서버가 Cloudflare WAF를 통해 서버 사이드 WebSocket 핸드셰이크를 차단하고 있음.
*   **IMF 데이터 지연:** IMF 측의 데이터 가공 상황에 따라 화요일 이후에도 며칠간 데이터가 순차적으로 채워질 수 있습니다. (기본 10일치 적재 옵션이 이를 자동으로 해결합니다.)
