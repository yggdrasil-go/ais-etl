...
*   **실시간 수집:** [aisstream.io](https://aisstream.io) (WebSocket)
    *   **글로벌 모니터링:** `bbox.json`에 정의된 전 세계 28개 주요 요충지의 실시간 트래픽 수집 가능.
...
*   **`trino_manager.py`**: 조회 전용 Trino 연결 및 쿼리 실행 관리.
...
본 프로젝트는 `ais-etl` 가상환경을 권장합니다.
```bash
conda activate ais-etl
...
- `TRINO_URL`, `TRINO_PORT`: 조회용 Trino 접속 정보
...
# Trino 적재 상태 확인
python -c "from src.trino_manager import TrinoManager; tm = TrinoManager(); print(tm.fetch_data('SELECT * FROM portwatch.ports ORDER BY updated_at DESC LIMIT 5'))"
```
...
## ⚠️ 이슈 리포트
*   **Arrow ChunkedArray Error:** `pd.concat` 후 적재 시 발생하는 Arrow 변환 오류는 `to_dict('records')`를 통해 해결되었습니다.
*   **AISStream Cloudflare Issue:** 서버 사이드 WebSocket 접속 시 Cloudflare WAF 차단 주의.
